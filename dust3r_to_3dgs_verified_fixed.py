...
def write_cameras_bin(sparse_out, cameras):
    """
    Write cameras.bin in official COLMAP little-endian fixed-width format.
    ...
    Layout:
        ...
        repeated num_cameras times:
            ...
            uint64 width
            uint64 height
            float64 params[num_params]
    """
    with open(os.path.join(sparse_out, "cameras.bin"), "wb") as f:
        f.write(struct.pack("<Q", len(cameras)))
        for cid, w, h, fx, fy, cx, cy, _ in cameras:
            # COLMAP model_id = 1 -> PINHOLE, params = [fx, fy, cx, cy]
            f.write(struct.pack("<IiQQ", int(cid), 1, int(w), int(h)))
            f.write(struct.pack("<4d", float(fx), float(fy), float(cx), float(cy)))


def write_images_bin(sparse_out, poses, cameras):
    """
    Write images.bin in official COLMAP little-endian fixed-width format.

    Layout:
        uint64 num_images
        repeated num_images times:
            uint32 image_id
            float64 qw, qx, qy, qz
            float64 tx, ty, tz
            uint32 camera_id
    ...
    We currently do not export 2D observations, so num_points2D = 0.
    """
    with open(os.path.join(sparse_out, "images.bin"), "wb") as f:
        ...
        for i, pose in enumerate(poses):
            ...
            f.write(struct.pack("<4d", float(q[3]), float(q[0]), float(q[1]), float(q[2])))  # wxyz
            f.write(struct.pack("<3d", float(t[0]), float(t[1]), float(t[2])))
            f.write(struct.pack("<I", i + 1))
            f.write(name_bytes)
...
def write_points3d_bin(sparse_out, pts_all, cols_all):
    """
    Write points3D.bin in official COLMAP little-endian fixed-width format.

    Layout:
        uint64 num_points3D
        repeated num_points3D times:
            uint64 point3D_id
            float64 x, y, z
            uint8   r, g, b
            float64 error
            uint64  track_length
            repeated track_length times:
                uint32 image_id
                uint32 point2D_idx

    We currently do not export tracks, so track_length = 0.
    """
    with open(os.path.join(sparse_out, "points3D.bin"), "wb") as f:
        f.write(struct.pack("<Q", len(pts_all)))
